...
export default function UsersTable({ data, supervisors, campaigns, onRefresh }: UsersTableProps) {
    ...
    return (
        <div className="overflow-x-auto border rounded-md border-gray-200 shadow-sm bg-white">
            <table className="w-full text-left text-sm text-gray-700">
                ...
                <tbody className="divide-y divide-gray-100">
                    {data.length === 0 ? (
                        <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400 font-medium text-[12px] uppercase tracking-widest">Sin registros en la base actual</td></tr>
                    ) : data.map((item) => (
                        <tr key={item.id} className="hover:bg-blue-50/30 transition-all group">
                            <td className="px-4 py-2.5">
                                <div className="flex items-center gap-2">
                                    <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center border border-slate-200 text-slate-400 group-hover:bg-blue-100 group-hover:border-blue-200 group-hover:text-blue-500 transition-colors">
                                        <UserIcon className="w-4 h-4" />
                                    </div>
                                    <div className="flex flex-col">
                                        <span className="text-[12px] font-bold text-gray-800 uppercase tracking-tight">{item.first_name} {item.last_name}</span>
                            ...
                            <td className="px-4 py-2.5">
                                {item.supervisor_id ? (
                                    ...
                                    <span className="text-gray-400 italic text-[11px] tracking-tight">Sin asignar</span>
                                )}
                            </td>
                            <td className="px-4 py-2.5 text-[12px] font-bold text-gray-500 uppercase">
                                {campaigns.find(c => c.id === item.default_campaign_id)?.name || <span className="text-gray-300 font-normal">--</span>}
                            </td>
                            <td className="px-4 py-2.5 text-center">
                                {item.vicidial_user ? (
                                    <span className="text-[12px] font-mono font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100 uppercase">{item.vicidial_user}</span>
                                ) : (
                            ...
                            <td className="px-4 py-2.5 text-center">
                                {item.is_blocked ? (
                                    <span className="bg-red-100 text-red-800 border border-red-200 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase block w-fit mx-auto">BLOQUEADO</span>
                                ) : item.is_active ? (
                    ...
                    <div className="flex-1 p-5 space-y-5">
                        ...
                        <div className="space-y-4">
                            <div className="border-b border-gray-100 pb-1.5">
                                <p className="text-[12px] font-black text-gray-400 uppercase tracking-[0.2em]">01. Identidad Administrada</p>
                            </div>
                            <div className="grid grid-cols-12 gap-4">
                                <div className="col-span-6 space-y-1">
                                    <label className="text-[12px] font-bold text-gray-500 uppercase tracking-wider pl-1">Nombre Completo</label>
                                    <input disabled readOnly value={editingItem ? `${editingItem.first_name} ${editingItem.last_name}` : ''} className="w-full bg-gray-50 text-gray-400 border border-gray-200 rounded-md px-3 h-9 text-xs font-bold cursor-not-allowed uppercase" />
                                </div>
                                <div className="col-span-6 space-y-1">
                                    <label className="text-[12px] font-bold text-gray-500 uppercase tracking-wider pl-1">Email / Usuario System</label>
                                    <input disabled readOnly value={editingItem?.email || ''} className="w-full bg-gray-50 text-gray-400 border border-gray-200 rounded-md px-3 h-9 text-xs font-bold cursor-not-allowed uppercase" />
                                </div>
                                <div className="col-span-12 space-y-1">
                                    <label className="text-[12px] font-bold text-gray-500 uppercase tracking-wider pl-1 font-mono">Rol de Sistema</label>
                                    <div className="flex items-center gap-2 h-9 px-3 bg-blue-50/30 border border-blue-100 rounded-md text-blue-700 text-xs font-bold uppercase tracking-tight">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-12 gap-4">
                                ...
                                <div className="col-span-6 space-y-1">
                                    <label className="text-[12px] font-bold text-gray-700 uppercase tracking-wider pl-1">Jerarquía (Supervisor)</label>
                                    <select name="supervisor_id" defaultValue={editingItem?.supervisor_id || ''} className="w-full bg-white border border-gray-300 rounded-md px-3 h-9 text-xs font-bold text-gray-900 focus:border-blue-600 focus:ring-2 focus:ring-blue-100 outline-none transition-all uppercase appearance-none">
                                ...
                                <div className="col-span-6 space-y-1">
                                    ...
                                    <select name="default_campaign_id" defaultValue={editingItem?.default_campaign_id || ''} className="w-full bg-white border border-gray-300 rounded-md px-3 h-9 text-xs font-bold text-gray-900 focus:border-blue-600 focus:ring-2 focus:ring-blue-100 outline-none transition-all uppercase appearance-none">
                                        <option value="">Sin Asignación</option>
                                        {uniqueCampaigns.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                    </select>
                                </div>
                                <div className="col-span-12 space-y-1">
                                    <label className="text-[12px] font-bold text-gray-700 uppercase tracking-wider pl-1 font-bold text-blue-600">Especialidades de Producto (Skills)</label>
                                    ...
                                    <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider mt-1">Selecciona todos los productos que este usuario está capacitado para gestionar</p>
                                </div>
                                <div className="col-span-6 space-y-1">
                                    <label className="text-[12px] font-bold text-gray-700 uppercase tracking-wider pl-1 text-orange-600 font-black">Límite WIP Individual</label>
                                    <input
                                        type="number"
                                        name="custom_max_tasks"
                                        placeholder="Usa límite del rol"
                                        defaultValue={editingItem?.custom_max_tasks}
                                        className="w-full border border-gray-300 rounded-md px-3 h-9 text-xs font-bold text-gray-900 focus:border-orange-500 focus:ring-2 focus:ring-orange-100 outline-none transition-all bg-white"
                                    />
                                    <p className="text-[9px] text-orange-400 font-bold uppercase">Sobrescribe el límite global del rol para este usuario</p>
                                </div>
                                <div className="col-span-6 space-y-1">
                                    <label className="text-[12px] font-bold text-gray-700 uppercase tracking-wider pl-1">Fecha Efectiva Alta</label>
                                    <input type="date" name="join_date" defaultValue={editingItem?.join_date ? editingItem.join_date.split('T')[0] : ''} className="w-full bg-white border border-gray-300 rounded-md px-3 h-9 text-xs font-bold text-gray-900 focus:border-blue-600 focus:ring-2 focus:ring-blue-100 outline-none transition-all" />
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="flex items-center justify-between p-6 bg-gray-50 border-t border-gray-200 shrink-0 px-8">
                        <div className="flex items-center gap-2">
                            <div className={`w-2.5 h-2.5 rounded-full ${editingItem?.is_active ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`} />
                            <span className="text-[12px] font-black text-gray-500 uppercase tracking-widest">{editingItem?.is_active ? 'USUARIO ACTIVO EN SISTEMA' : 'USUARIO INACTIVO'}</span>
                        </div>
                        <div className="flex gap-4">
                            <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 h-10 text-xs font-bold text-gray-500 uppercase hover:text-gray-900 hover:bg-gray-100 rounded-md transition-all">Cancelar</button>
